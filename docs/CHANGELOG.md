...
## 💡 LEÇONS APPRISES
### Sprint 17 : Sawt El Moustahlik (Multimodalité & Orchestration)
- **Séquençage de Fusion & Régressions de Prompt** : Lors de la fusion de plusieurs PR techniques impactant le même fichier (ex: `src/rag_engine.py`), le passage d'une version de prompt (PBI-1701 Multimodal) à une autre (Migration GPT-4o-Audio) a provoqué l'écrasement silencieux des consignes de persona. **Leçon** : Toujours relancer les tests de qualité RAG (`tests/test_rag_quality.py`) entre chaque fusion, et non seulement les tests unitaires locaux, pour détecter la dérive du persona LLM avant la livraison.
- **Hygiène de la Racine & Résidus de Dev** : L'accumulation de fichiers binaires (`.opus`, `.mp3`) et de scripts de test à la racine pollue le repo et peut fausser les tests globaux ou la CI. Le nettoyage systématique des fichiers temporaires générés par les tests multimédia est une priorité post-Sprint.
