...
- [x] Définir un seuil de similarité vectorielle (ex: 0.6) dans le `JumiaReRanker`. (Affiné à 0.8 pour respecter le DoD).
...
- **Logique** : Ajouter un filtre `node_with_score.score >= 0.6` au début de la méthode `_postprocess_nodes`.
- **Validation** : Test manuel avec la requête "Crème visage" et vérification de l'absence de produits informatiques.

## ✅ DEFINITION OF DONE (DoD)
- Le seuil de 0.6 (0.8 effectif) est appliqué et fonctionnel.
