...
describe("rewrite", () => {
  ...
  describe("elixir / phoenix / ash", () => {
    ...
    test("rewrites mix credo", () => {
      expect(rewrite("mix credo")).toBe("rtk lint mix credo")
    })
    test("rewrites mix format", () => {
      expect(rewrite("mix format")).toBe("rtk format mix format")
    })
    test("rewrites mix dialyzer", () => {
      expect(rewrite("mix dialyzer")).toBe("rtk err mix dialyzer")
    })
